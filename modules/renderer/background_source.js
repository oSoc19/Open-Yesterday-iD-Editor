        ...
        return (
            template
                ...
                .replace(/\{switch:([^}]+)\}/, function(s, r) {
                    ...
                    return subdomains[
                        (coord[0] + coord[1]) % subdomains.length
                    ];
                })
                .replace('{u}', function() {
                    var u = '';
...
rendererBackgroundSource.Bing = function(data, dispatch) {
    ...
    data.template =
        'https://ecn.t{switch:0,1,2,3}.tiles.virtualearth.net/tiles/a{u}.jpeg?g=587&mkt=en-gb&n=z';
    ...
    var key =
        'Ak5oTE46TUbjRp08OFVcGpkARErDobfpuyNKa-W2mQ8wbt1K1KL8p1bIRwWwcF-Q'; // iD

    var url =
        'https://dev.virtualearth.net/REST/v1/Imagery/Metadata/Aerial?include=ImageryProviders&key=' +
        key;
    var cache = {};
    var inflight = {};
    var providers = [];
...
rendererBackgroundSource.Esri = function(data) {
    ...
    esri.getMetadata = function(center, tileCoord, callback) {
        ...
        if (esri.id === 'EsriWorldImagery') {
            ...
            url =
                'https://serviceslab.arcgisonline.com/arcgis/rest/services/Clarity_World_Imagery/MapServer/';
        }

        url +=
            metadataLayer +
            '/query?returnGeometry=false&geometry=' +
            centerPoint +
        ...
        if (metadataLayer === 99) {
            ...
            d3_json(url)
                .then(function(result) {
                    ...
                    var captureDate = localeDateString(
                        result.features[0].attributes.SRC_DATE2
                    );
                    vintage = {
                        start: captureDate,
                        end: captureDate,
                    ...
                    metadata = {
                        ...
                        description: clean(
                            result.features[0].attributes.NICE_DESC
                        ),
                        resolution: clean(
                            result.features[0].attributes.SRC_RES
                        ),
