...
export function uiToolAddFavorite(context) {
    ...
    tool.render = function(selection) {
        context.on('enter.editor.favorite', function(entered) {
            selection
        ...
        context.on('exit.editor.favorite', function(exited) {
        ...
        function update() {
            ...
            var modes = items.map(function(d, index) {
                ...
                var markerClass =
                    ...
                    d.geometry +
                    ' add-preset-' +
                    presetName.replace(/\s+/g, '_') +
                    '-' +
                    d.geometry +
                    ' add-' +
                    d.source; // replace spaces with underscores to avoid css interpretation
                if (d.preset.isFallback()) {
                    markerClass += ' add-generic-preset';
                }

                var supportedGeometry = d.preset.geometry.filter(function(
                    geometry
                ) {
                    return (
                        ['vertex', 'point', 'line', 'area'].indexOf(
                            geometry
                        ) !== -1
                    );
                });
                ...
                if (
                    vertexIndex !== -1 &&
                    supportedGeometry.indexOf('point') !== -1
                ) {
                ...
                if (supportedGeometry.length !== 1) {
                    if (d.preset.setTags({}, d.geometry).building) {
                        ...
                        tooltipTitleID =
                            'modes.add_preset.' +
                            context.presets().fallback(d.geometry).id +
                            '.title';
                    }
                }
                ...
                protoMode.description = t(tooltipTitleID, {
                    feature: '<strong>' + presetName + '</strong>'
                });
            ...
            var buttons = selection
                .selectAll('button.add-button')
                .data(modes, function(d, index) {
                    return d.button + index;
                });
            ...
            buttonsEnter.each(function(d) {
                d3_select(this).call(
                    uiPresetIcon(context)
                        .geometry(
                            d.geometry === 'point' &&
                                !d.preset.matchGeometry(d.geometry)
                                ? 'vertex'
                                : d.geometry
                        )
                        .preset(d.preset)
                    ...
                    .on('end', function(d, index) {
                        ...
                        if (y > 50) {
                            ...
                            if (d.isFavorite()) {
                                context
                                    .presets()
                                    .removeFavorite(d.preset, d.geometry);
                                // also remove this as a recent so it doesn't still appear
                                context
                                    .presets()
                                    .removeRecent(d.preset, d.geometry);
                            }
                            ...
                            if (d.isFavorite()) {
                                context
                                    .presets()
                                    .moveFavorite(index, targetIndex);
                            }
                        }
                    })
            );
